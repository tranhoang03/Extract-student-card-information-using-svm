...
def download_and_extract_models():
    ...
    if response.status_code == 200:
        with zipfile.ZipFile(io.BytesIO(response.content)) as zip_ref:
            model_files = {name: zip_ref.read(name) for name in zip_ref.namelist()}
        st.write(f"File đã được giải nén: {list(model_files.keys())}")
        return model_files
    else:
...
model_files = download_and_extract_models()

# Kiểm tra xem các mô hình đã được giải nén chưa
if model_files:
    MODEL_PATHS = {
        'hoten': model_files.get('svm_hoten.pkl'),
        'ngaysinh': model_files.get('svm_ngaysinh.pkl'),
        'lop': model_files.get('svm_lop.pkl'),
        'msv': model_files.get('svm_masinhvien.pkl'),
        'nienkhoa': model_files.get('svm_nienkhoa.pkl'),
        'anhthe': model_files.get('svm_anhthe.pkl')
    }

    # Kiểm tra sự tồn tại của các mô hình
    for model, file_content in MODEL_PATHS.items():
        if file_content is None:
            st.error(f"Không tìm thấy mô hình: {model}")
        else:
            st.write(f"Đã tải mô hình: {model}")
